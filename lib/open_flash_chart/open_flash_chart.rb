class Graph
  ...
  def format_output(function, values)
    if @output_type == 'js'
      "so.addVariable('#{function}','#{values}');"
    else
      "&#{function}=#{values}&"
    end
  end
  ...
  def render
    ...
    if @output_type == 'js'
      ...
      temp << '<script type="text/javascript" src="/plugin_assets/redcase/javascripts/swfobject.js"></script>'
      temp << '<script type="text/javascript">'
      temp << 'var so = new SWFObject("/plugin_assets/redcase/open-flash-chart.swf", "ofc", "' + @width.to_s + '", "' + @height.to_s + '", "9", "#FFFFFF");'
...
def _ofc(width, height, url, use_swfobject, base='/', set_wmode_transparent=false)
  ...
  if use_swfobject
    ...
    out << 'var so = new SWFObject("/plugin_assets/redcase/open-flash-chart.swf", "' + obj_id + '","' + width.to_s + '","' + height.to_s + '", "9", "#FFFFFF");'
  ...
  out << '<param name="movie" value="/plugin_assets/redcase/open-flash-chart.swf?' + width.to_s + '&height=' + height.to_s + '&data=' + url + '" />'
  out << '<param name="quality" value="high" />'
  out << '<param name="bgcolor" value="#FFFFFF" />'
  out << '<param name="wmode" value="transparent" />' if set_wmode_transparent
  out << '<embed src="/plugin_assets/redcase/open-flash-chart.swf?data=' + url + '" quality="high" bgcolor="#FFFFFF" width="' + width.to_s + '" height="' + height.to_s + '" name="' + obj_id + '" align="middle" allowScriptAccess="sameDomain" '
...
def open_flash_chart_object_str(width, height, url, use_swfobject=true, base='/', set_wmode_transparent=false)
  _ofc(width, height, url, use_swfobject, base, set_wmode_transparent)
end
