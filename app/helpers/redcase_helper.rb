...
module RedcaseHelper
  ...
  def test_suite_unsorted(project)
    test_suite_root(project).children.detect { |o| o.name == '.Unsorted' }
  end

  # Returns test suite linked to the project for 'unsorted' test cases
  def test_suite_obsolete(project)
    test_suite_root(project).children.detect { |o| o.name == '.Obsolete' }
  end
  ...
  def execution_environment_default(project)
    env = ExecutionEnvironment.find_by_project_id(project.id)
    if env.nil?
  ...
  def get_plugin_tabs(project)
    if Version.find_all_by_project_id(project.id).length == 0
      tabs = [{ :name => 'Management', :partial => 'redcase/management', :label => :label_test_case_management }]
    elsif !User.current.allowed_to?(:edit_test_cases, project)
      tabs = [{ :name => 'Report', :partial => 'redcase/report', :label => :label_test_case_report }]
    else
      tabs = [
  ...
  def get_test_suite_counts_trees(test_case)
    ...
    test_case.execution_suites.each do |x|
  ...
  def update_execution_suite_link(icon, act, size = '20x20')
    #link_to_remote image_tag(icon, :plugin => 'redcase', :size => size, :style => 'vertical-align: middle'),
    #               :url => { :action => 'update_exelists',
